...
impl MongoRepo {
    pub fn init() -> Self {
        dotenv().ok();
        let mongodb_uri = env::var("DATABASE_URL").expect("DATABASE_URL must be set.");
        let database_name =
            env::var("MONGO_INITDB_DATABASE").expect("MONGO_INITDB_DATABASE must be set.");
        let user_collection =
            env::var("MONGODB_USER_COLLECTION").expect("MONGODB_USER_COLLECTION must be set.");

        let client = Client::with_uri_str(mongodb_uri).unwrap();
        let db = client.database(&database_name);
        let col = db.collection::<User>(&user_collection);
